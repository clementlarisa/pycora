"""NonlinearSys reach from CORA @nonlinearSys main reach loop.

Implements the basic ``lin`` algorithm (ie omits ``linRem`` refinement,set
splitting,adaptive step size). Each step:

  1. Linearize x_dot = f(x, u) at the center of the current reach set
...
References:
  CORA/contDynamics/@nonlinearSys/{linearize.m, initReach.m} chapter 3 in PhD thesis
"""
...
class NonlinearSys:
    """Continuous-time nonlinear system x_dot = f(x, u).

    Parameters
    ----------
    f : callable f(x, u) -> R^n
        Dynamics. JAX-traceable
    n_x : int
        State dimension.
    n_u : int
        Input dimension.
    ...
    def reach(
        ...
        for k in range(n_steps):
            ...
            # CORA uses the one-step over-approximation for this
            # simplistically we use R_current itself, which is a slightly looser
            # bound but sound.
            ...
            # since we linearize x_dot ~ A*(x - x*) + B*(u - u*) + f(x*, u*),
            # we re-formulate as x_dot = A*x + B*u + (f(x*, u*) - A*x* - B*u*).
            # The total "constant input" passed to LinearSys.one_step is the
            # constant term plus B mapped offsets. shift
            ...
            # one_step: x_dot ~ A*x_shifted + B*u_shifted + c0
